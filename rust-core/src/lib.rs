...
pub mod prelude {
    pub use crate::ir::{IrCommand, IrBatch};
    pub use crate::platform::{
        PlatformBridge, PlatformId, PlatformCapability,
        NativeHandle, ViewType, PropValue, PropsDiff,
    };
    pub use crate::layout::{LayoutEngine, LayoutStyle, ComputedLayout};
    pub use crate::events::{InputEvent, PointerEvent, KeyboardEvent};
    pub use crate::tree::{ShadowTree, NodeId};
    pub use crate::scheduler::{Scheduler, Priority};
    pub use crate::navigation::{Navigator, NavigationAction, NavigationEvent};
    pub use crate::accessibility::{AccessibilityInfo, AccessibilityRole, FocusManager};
    pub use crate::bridge::{SyncCall, SyncResult, AsyncCall, NativeCallback};
...
use bridge::{SyncCall, SyncResult, AsyncCall};
...
impl Engine {
    ...
    fn apply_ir_to_tree(
        &mut self,
        batch: &ir::IrBatch,
    ) -> Result<Vec<tree::NodeId>, EngineError> {
        let mut dirty = Vec::new();

        for cmd in &batch.commands {
            match cmd {
                ir::IrCommand::CreateNode { id, view_type, props, style } => {
                ...
                ir::IrCommand::InsertBefore { parent, child, before } => {
        ...
        for &node_id in dirty_nodes {
                ...
                Some(n) => (
                    ...
                    n.parent.and_then(|pid| {
                        self.tree.get(pid).and_then(|p| {
                            p.native_handle.map(|h| (h, p.children.iter().position(|&c| c == node_id).unwrap_or(0)))
                        })
                    }),
                    n.native_handle,
                ),
            ...
            if let Some(handle) = self.tree.get(node_id).and_then(|n| n.native_handle) {
                ...
                if !props_diff.is_empty() {
                    self.platform.update_view(handle, &props_diff)
                ...
                if let Some(layout) = self.layout.get_computed(node_id) {
                    ...
                    position_props.set("frame", PropValue::Rect {
                        x: layout.x,
                        y: layout.y,
                        width: layout.width,
                        height: layout.height,
                    });
                    self.platform.update_view(handle, &position_props)
                        .map_err(|e| EngineError::Platform(e.to_string()))?;
                }
            }
        }
    ...
    pub fn handle_sync(&self, call: &SyncCall) -> SyncResult {
        match call {
            SyncCall::Measure { node_id } => {
                match self.layout.get_computed(*node_id) {
                    Some(layout) => SyncResult::from_layout(layout),
                    None => SyncResult::NotFound,
                }
            }

            SyncCall::IsFocused { node_id } => {
                SyncResult::Bool {
            ...
            SyncCall::SupportsCapability { capability } => {
                ...
                SyncResult::Bool {
                    value: cap.map_or(false, |c| self.platform.supports(c)),
                }
            }
            ...
            SyncCall::GetAccessibilityRole { node_id } => {
                ...
                match self.tree.get(*node_id) {
                    Some(node) => {
                        ...
                        SyncResult::Role { role: role.to_string() }
                    }
                    None => SyncResult::NotFound,
                }
            }
            ...
            SyncCall::NodeExists { node_id } => {
                SyncResult::Bool {
                    value: self.tree.get(*node_id).is_some(),
                }
            }

            SyncCall::GetChildCount { node_id } => {
                match self.tree.get(*node_id) {
                    Some(node) => SyncResult::Int { value: node.children.len() as u64 },
                    None => SyncResult::NotFound,
                }
            }
            ...
            SyncCall::MeasureText { text, style, max_width } => {
                ...
                let metrics = self.platform.measure_text(text, &platform_style, *max_width);
            ...
            SyncCall::GetFocusedNode => {
                SyncResult::NodeIdResult {
            ...
            SyncCall::GetActiveRoute => {
                match self.navigator.active_screen() {
