...
|       |
|       +-- bridge.ts           [290 ln]  Hybrid bridge (sync/async split)
|                                         Type-safe sync reads + async mutations,
|                                         React hooks (useLayout, useNavigation),
...
| Hybrid bridge (Rust) | bridge.rs | 310 | 4 |
| Host config (TS) | host-config.ts | 482 | — |
| JS scheduler (TS) | scheduler.ts | 221 | — |
| Hybrid bridge (TS) | bridge.ts | 290 | — |
| FlatBuffer schema | ir.fbs | 191 | — |
| **Total** | **14 source files** | **4,433** | **19** |
...
### 13. Hybrid Bridge (Rust)

**File:** `rust-core/src/bridge.rs` (310 lines)

The Rust side of the sync/async boundary. This module enforces the architectural rule that no mutation can happen in the sync path and no read blocks on the async path.

**SyncCall enum** — exhaustive list of what JS can read synchronously. If a method isn't in this enum, it can't be called synchronously. This is compile-time enforcement.

**AsyncCall enum** — exhaustive list of mutations. All of these go through the scheduler and are processed on the next frame.

**C FFI entry points** — Three `#[no_mangle] extern "C"` functions that platform bridges call via JSI:
...
4 tests cover: screen size reads, back navigation state, text measurement, and capability queries.

---

### 14. Hybrid Bridge (TypeScript)

**File:** `sdk/src/bridge.ts` (290 lines)

The TypeScript API that components and the host config use. Provides type-safe methods split into two groups:

**Sync methods** (return immediately): `measure()`, `measureText()`, `isFocused()`, `getFocusedNode()`, `getScrollOffset()`, `supports()`, `getScreenSize()`, `canGoBack()`, `getActiveRoute()`

**Async methods** (enqueue, return void): `applyCommit()`, `navigate()`, `setFocus()`, `moveFocus()`, `announce()`

**React hooks**: `useLayout(nodeId)`, `usePlatformCapability(capability)`, `useScreenSize()`, `useNavigation()` — these wrap sync bridge calls in React-friendly interfaces.

**NativeEngine interface** — the contract that platform-specific code must implement. Two methods: `syncCall(json): string` and `asyncCall(json): void`. iOS implements this via JSI, web via WASM, desktop via FFI.
...
JS Thread (Hermes / V8)
  ...
  Produces: IrBatch (async) + sync queries
       ...
       | applyCommit()            | measure(), isFocused(),
       | navigate()               | getScreenSize(), etc.
       | setFocus()               | Reads only, no mutations
       v                          v
Rust Core Thread
  Shadow tree, IR decode, scheduler, navigation, DevTools
  Async: enqueue batch -> process on next frame
  Sync: read from already-computed state -> return immediately
...
**Key constraints:**
- Layout can run on a background thread, but native view operations must execute on the platform's main thread.
- Sync calls NEVER trigger mutations or layout. They read from the last-computed state.
- Async calls NEVER block the JS thread. They enqueue work for the next frame.
- This hybrid model matches Flutter's architecture: mutations are async, reads are sync.
...
### 7. Hybrid Sync/Async Bridge (Not Fully Synchronous)

The engine is a batched, frame-driven system — not a synchronous call system. Fully sync would break the scheduler, block the JS thread during layout, and turn "one commit per frame" back into "one call per mutation" (the exact failure mode of React Native's old bridge).

The split is enforced at the type level in both Rust and TypeScript:
...
Rule: No UI mutation is allowed in the sync path. Sync reads from already-computed state. Async enqueues for the next frame.

**Files:** `rust-core/src/bridge.rs` (Rust enforcement + FFI entry points) and `sdk/src/bridge.ts` (TypeScript API + React hooks).
